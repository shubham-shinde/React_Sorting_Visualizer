...
const creatGrid = (nCol) => {
  let width = window.screen.availWidth;
  let height = window.screen.availHeight;
  const col_height = height/nCol;
  const nRow = width/col_height;
  const data = { row: 1, column: 1, start: false, end: false, clog: false, checked: false, path: false, queue: false }
  let grid = [];
  for(let i=1; i<=nCol; i++) {
    const arr = [];
    for(let j=1; j<=nRow; j++) {
      arr = [...arr, {...data, row:i, column:j}]
    }
    grid = [...grid, [...arr]];
  }
  return grid;
